...
const SlotConfigModal = ({ 
  ...
  const StatusIcon = ({ status }) => {
    if (status === 'ok') return <CheckCircle2 size={12} className="status-check-ok" />;
    if (status === 'error') return <AlertTriangle size={12} className="status-check-error" />;
    return <Clock size={12} className="status-check-pending" />;
  };
  ...
  return (
    <Modal
      ...
      size="lg"
    >
      <div className="slot-config-container">
        {/* Header Summary */}
        <div className="slot-config-section subject-summary-card">
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {isValidating && (
                  <span className="flex items-center gap-1.5 text-[9px] font-black text-blue-500 uppercase validating-pulse">
        ...
        <div className="availability-checklist">
          {Object.entries(checklistStatus).map(([key, info]) => (
            <div key={key} className="checklist-item">
              ...
              <div className="checklist-status">
                ...
                <span className={`status-check-${info.status}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            <div className="slot-config-section">
                ...
                {DAYS.map(day => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={isDisabled}
                      className={`day-pill ${isSelected ? 'selected' : ''} ${isDisabled ? 'disabled' : ''} ${isConflict ? 'conflict-glow' : ''}`}
                    ...
                    >
                      <span>{day}</span>
                      {(formData.professor || isConflict) && <div className={`avail-dot ${status}`} />}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="mt-6 duration-meter-box">
                <div className="duration-header">
                  <span className="duration-label">Total Selected Time</span>
                  <span className={`duration-value state-${durationInfo.status}`}>
                    {durationInfo.label}
                  </span>
                </div>
                <div className="duration-progress-bg">
                  <div 
                    className={`duration-progress-bar bg-${durationInfo.status}`}
                  ...
                  <div className="prof-micro-grid">
                    {DAYS.map(day => {
                      ...
                      return (
                        <div 
                          key={day} 
                          className={`micro-cell ${isActive ? 'active' : ''} ${isSelected ? 'selected' : ''}`}
