...
const EditSubjectModal = ({ isOpen, onClose, subject, programId, programName, onSuccess }) => {
    ...
    // UI state for sharing (auto-enabled if they already have other programs)
    ...
    useEffect(() => {
        const initForm = async () => {
            if (isOpen && subject) {
                ...
                // Available are all except the primary one
                setAvailablePrograms(allProgs.filter(p => p.id !== currentProgId));
                
                // Selected are those in program_ids except the primary one
                                        ...
                                        <div className="lg:col-span-8 space-y-8">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 space-y-4">
                                                    {/* Primary Program (Locked in Edit) */}
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center font-black text-xs text-indigo-600 shadow-sm border border-gray-100">
                                                                {subject.program_code?.substring(0,2).toUpperCase() || 'PC'}
                                                            </div>
                                                            <div>
                                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Primary Program</p>
                                                                <p className="text-sm font-bold text-gray-900">{subject.program_name || programName}</p>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className="bg-gray-50 rounded-[24px] border border-gray-100 p-6 h-full flex flex-col">
                                                ...
                                                <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-2 max-h-[400px]">
                                                    ...
                                                    {availableSubjects.filter(s => !selectedPrereqs.find(p => p.id === s.id)).map(subjectItem => (
                                                        ...
                                                        >
                                                            <div className="flex items-center justify-between">
                                                                <span className="text-xs font-bold text-gray-900 group-hover:text-indigo-700">{subjectItem.code}</span>
                                                                <Plus size={12} className="text-gray-300 group-hover:text-indigo-500" />
                                                            </div>
                                                            <p className="text-[9px] text-gray-400 truncate mt-1">{subjectItem.title}</p>
                                                        </button>
                                                    ))}
                                                    {(availableSubjects.length === 0 && !searching) && (
