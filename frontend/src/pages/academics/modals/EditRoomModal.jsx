...
const EditRoomModal = ({ isOpen, onClose, onSuccess, room }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await FacilitiesService.updateRoom(room.id, formData);
            onSuccess();
        } catch (err) {
            console.error(err);
            setErrors({ submit: err.response?.data?.message || 'Failed to update room' });
        } finally {
